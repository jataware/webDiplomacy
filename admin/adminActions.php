...
class adminActions extends adminActionsForms
{
	...
	public function drawType(array $params)
	{
		...
		switch ($newSetting) {
			case 1:
				$newSettingName = 'draw-votes-public';
				break;
			case 2:
				$newSettingName = 'draw-votes-hidden';
				break;
			default:
				throw new Exception(l_t("Invalid draw vote setting - enter 1 (public) or 2 (hidden)"));
		}
	...
	public function alterMessaging(array $params)
	{
		...
		switch ($newSetting) {
			case 1:
				$newSettingName = 'Regular';
				break;
			case 2:
				$newSettingName = 'PublicPressOnly';
				break;
			case 3:
				$newSettingName = 'NoPress';
				break;
			case 4:
				$newSettingName = 'RuleBookPress';
				break;
			default:
				throw new Exception(l_t("Invalid messaging setting; enter 1, 2, 3, or 4."));
		}
	...
	public function changePhaseLength(array $params)
	{
		...
		return l_t(
			'Process time changed from %s to %s. Next process time is %s.',
			libTime::timeLengthText($oldPhaseMinutes * 60),
			libTime::timeLengthText($Game->phaseMinutes * 60),
			libTime::text($Game->processTime)
		);
	}
	...
	public function banUserConfirm(array $params)
	{
		...
		return l_t(
			'Are you sure you want to ban %s (Reason: "%s")? This removed them from all their games, so be sure this is correct!',
			$User->username,
			$DB->msg_escape($params['reason'])
		);
	}
	...
	public function updateEmergencyDate(array $params)
	{
		...
		if ($setting >= 0) {
			$targetUser->updateEmergencyPauseDate($setting);
		}
