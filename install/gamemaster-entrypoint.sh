...
if mysql -u webdiplomacy -h mariadb -P 3306 --password=$MYSQL_ROOT_PASSWORD webdiplomacy -e "SHOW TABLES;" | grep -q 'w[Dd]_[Uu]ser' ; then
  echo "DB installed"
else
  echo "DB not installed, installing new DB"
  mysql -u webdiplomacy -h mariadb -P 3306 --password=$MYSQL_ROOT_PASSWORD webdiplomacy < $HOME/install/FullInstall/fullInstall.sql
  mysql -u webdiplomacy -h mariadb -P 3306 --password=$MYSQL_ROOT_PASSWORD webdiplomacy < $HOME/install/createBotAccounts.sql
  echo "DB created"
fi
